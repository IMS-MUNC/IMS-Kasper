...
const AddPurchaseModal = ({ onSuccess }) => {
  ...
  const purchaseSummary = selectedProducts.reduce((acc, product) => {
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center' }}>
                                      ...
                                      <div className="form-check form-switch ms-2 d-inline-flex align-items-center" style={{ verticalAlign: 'middle' }}>
                                        <input
                                          ...
                                          onChange={e => {
                                            const newType = e.target.checked ? 'Percentage' : 'Fixed';
                                            setSelectedProducts(prev => prev.map((p, idx) => idx === index ? { ...p, discountType: newType } : p));
                                          }}
                                        />
                                        <label className="form-check-label ms-1" htmlFor={`discountTypeSwitch-${index}`} style={{ fontSize: '12px' }}>
